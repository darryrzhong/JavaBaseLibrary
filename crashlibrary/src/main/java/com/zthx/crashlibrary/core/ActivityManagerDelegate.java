        ...
        switch (methodName)
        {
            case "finishActivity":
                ...
                Class<? extends Activity> clazz =
                    Recovery.getInstance().getMainPageClass();
                if (clazz == null)
                {
                    break;
                }
                int count = ActivityStackCompat
                    .getActivityCount(Recovery.getInstance().getContext());
                String baseActivityName = ActivityStackCompat
                    .getBaseActivityName(Recovery.getInstance().getContext());
                ...
                {
                    Intent intent =
                        new Intent(Recovery.getInstance().getContext(), clazz);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK
                        | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                    if (RecoveryUtil.isIntentAvailable(
                        Recovery.getInstance().getContext(),
                        intent))
                    {
