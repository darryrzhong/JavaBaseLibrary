...
import com.zthx.crashlibrary.R;
import com.zthx.crashlibrary.utils.OSUtils;
import com.zthx.crashlibrary.utils.RecoverySharedPrefsUtil;
...
public final class RecoveryActivity extends AppCompatActivity
{
    ...
    public boolean onOptionsItemSelected(MenuItem item)
    {
        ...
        else if (id == R.id.action_save)
        {
            ...
            Toast
                .makeText(this,
                    isSuccess ? "Save success!" : "Save failed!",
                    Toast.LENGTH_SHORT)
                .show();
        }
    ...
    private void initView()
    {
        ...
        {
            mScrollView
                .setPadding(0, RecoveryUtil.dp2px(getApplication(), 16), 0, 0);
        }
    }
...
//        Log.d("手机信息", OSUtils.getDeviceInfo(new Date(System.currentTimeMillis()),new Date(System.currentTimeMillis()),"java",this.getPackageName(),OSUtils.getAppVersion(this)));
    }
    ...
    private void setupEvent()
    {
                ...
                AlertDialog dialog =
                    new AlertDialog.Builder(RecoveryActivity.this)
                        .setTitle(getResources()
                            .getString(R.string.recovery_dialog_tips))
                        .setMessage(getResources()
                            .getString(R.string.recovery_dialog_tips_msg))
                        .setPositiveButton(
                            getResources()
                                .getString(R.string.recovery_dialog_sure),
                        ...
                        .setNegativeButton(
                            getResources()
                                .getString(R.string.recovery_dialog_cancel),
                            new DialogInterface.OnClickListener()
                            {
                                @Override
                                public void onClick(DialogInterface dialog,
        ...
        mCrashTipsTv.setText(String.format(
            getResources().getString(R.string.recovery_crash_tips_msg),
            RecoveryUtil.getAppName(this)));
        
        if (mExceptionData != null)
        {
            String type =
                mExceptionData.type == null ? "" : mExceptionData.type;
            String name = mExceptionData.className == null ? ""
                : mExceptionData.className;
            
            mExceptionTypeTv.setText(String.format(
                getResources().getString(R.string.recovery_exception_type),
                type.substring(type.lastIndexOf('.') + 1)));
            
            mClassNameTv.setText(String.format(
                getResources().getString(R.string.recovery_class_name),
                name.substring(name.lastIndexOf('.') + 1)));
            
            mMethodNameTv.setText(String.format(
                getResources().getString(R.string.recovery_method_name),
                mExceptionData.methodName));
            
            mLineNumberTv.setText(String.format(
                getResources().getString(R.string.recovery_line_number),
                mExceptionData.lineNumber));
        }
    ...
    {
        return getIntent().getBooleanExtra(RecoveryStore.IS_SHOW_DEBUG_INFO,
            false);
    }
    ...
    {
        boolean hasRecoverStack =
            getIntent().hasExtra(RecoveryStore.RECOVERY_STACK);
        return !hasRecoverStack
            || getIntent().getBooleanExtra(RecoveryStore.RECOVERY_STACK, true);
    }
    ...
    private Intent getRecoveryIntent()
    {
        boolean hasRecoverIntent =
            getIntent().hasExtra(RecoveryStore.RECOVERY_INTENT);
        if (!hasRecoverIntent)
            return null;
    ...
    private ArrayList<Intent> getRecoveryIntents()
    {
        boolean hasRecoveryIntents =
            getIntent().hasExtra(RecoveryStore.RECOVERY_INTENTS);
    ...
    {
        String date = RecoveryUtil.getDateFormat()
    ...
    private void setDisplayHomeAsUpEnabled(boolean enabled)
    {
        ...
        final ImageButton navButton = (ImageButton) Reflect.on(Toolbar.class)
            .field("mNavButtonView")
            .get(mToolbar);
        if (navButton != null)
        {
            if (enabled)
