...
import com.zthx.crashlibrary.utils.DefaultHandlerUtil;
...
final class RecoveryHandler implements Thread.UncaughtExceptionHandler
{
    ...
    static RecoveryHandler newInstance(
    ...
    public synchronized void uncaughtException(Thread t, Throwable e)
    {
        ...
        else
        {
            ...
            if (mCallback != null)
            {
                ...
                mCallback.exception(exceptionType,
                    throwClassName,
                    throwMethodName,
                    throwLineNumber);
            ...
            if (!DefaultHandlerUtil.isSystemDefaultUncaughtExceptionHandler(
    ...
    RecoveryHandler setCallback(ICrashCallback callback)
    {
        mCallback = callback;
        return this;
    }
    ...
    private void startRecoverActivity()
    {
        ...
        intent.setClass(Recovery.getInstance().getContext(),
            RecoveryActivity.class);
        intent.addFlags(
            Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK
                | Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS);
        if (RecoveryStore.getInstance().getIntent() != null) {
            intent.putExtra(RecoveryStore.RECOVERY_INTENT,
                RecoveryStore.getInstance().getIntent());
        }
        if (!RecoveryStore.getInstance().getIntents().isEmpty()) {
            intent.putParcelableArrayListExtra(RecoveryStore.RECOVERY_INTENTS,
                RecoveryStore.getInstance().getIntents());
        }
        intent.putExtra(RecoveryStore.RECOVERY_STACK,
            Recovery.getInstance().isRecoverStack());
        intent.putExtra(RecoveryStore.IS_DEBUG,
            Recovery.getInstance().isDebug());
        intent.putExtra(RecoveryStore.IS_SHOW_DEBUG_INFO,
            Recovery.getInstance().isShowDebug());
    ...
    private void startRecoverService()
    {
        ...
        intent.putExtra(RecoveryService.RECOVERY_SILENT_MODE_VALUE,
